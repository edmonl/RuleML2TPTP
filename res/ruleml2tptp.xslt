...
<xsl:template match="r:Assert">
  <xsl:apply-templates mode="top">
    <xsl:with-param name="formula-source" select="local-name()" as="xs:string"/>
  </xsl:apply-templates>
</xsl:template>
...
<!--
     We do not care about the formula is for Assert or for Query.
     That is the schema thing.
-->
...
<xsl:template match="r:formula" mode="top">
  ...
  <xsl:param name="formula-source" required="yes" as="as:string"/>

  <xsl:text>fof(</xsl:text>
  <!-- Formula name. -->
  <xsl:value-of select="concat(lower-case($formula-source), '_',
                        $act-index, '_',
                        count(preceding-sibling::*[name() = r:formula]) + 1)"/>
  ...
  <xsl:choose>
    <xsl:when test="r:formula">
      ...
      <xsl:for-each select="r:formula">
        <xsl:if test="not(. is r:formula[1])">
          <xsl:text>&amp;</text>
        </xsl:if>
        <xsl:apply-templates/>
      </xsl:for-each>
...
<xsl:template name="declare-list">
  <xsl:for-each select="r:declare">
    <xsl:if test="not(. is r:declare[1])">
      <xsl:test>, <xsl:test>
    </xsl:if>
    <xsl:apply-templates/>
  </xsl:for-each>
